...
/**
 Styles of DRPickerView.
 
 - Wheel: Style with 3D appearance like UIPickerView.
 - Flat:  Flat style.
 */
public enum DRPickerViewStyle {
    case Wheel
    case Flat
}
...
*/
@objc public protocol DRPickerViewDataSource {
    func numberOfItemsInPickerView(pickerView: DRPickerView) -> Int
    optional func pickerView(pickerView: DRPickerView, titleForItem item: Int) -> String
}
...
*/
@objc public protocol DRPickerViewDelegate: UIScrollViewDelegate {
    optional func pickerView(pickerView: DRPickerView, didSelectItem item: Int)
    optional func pickerView(pickerView: DRPickerView, marginForItem item: Int) -> CGSize
    optional func pickerView(pickerView: DRPickerView, configureLabel label: UILabel, forItem item: Int)
}

// MARK: - Private Classes and Protocols
// MARK: DRCollectionViewLayoutDelegate
/**
Private. Used to deliver the style of the picker.
*/
private protocol DRCollectionViewLayoutDelegate {
    func pickerViewStyleForCollectionViewLayout(layout: DRCollectionViewLayout) -> DRPickerViewStyle
}

// MARK: DRCollectionViewCell
/**
Private. A subclass of UICollectionViewCell used in DRPickerView's collection view.
*/
private class DRCollectionViewCell: UICollectionViewCell {
    var label: UILabel!
    ...
    func initialize() {
        ...
        self.label = UILabel(frame: CGRect(x: 0, y: 0, width: UIScreen.mainScreen().bounds.width / 3, height: 45))
...
// MARK: DRCollectionViewLayout
/**
Private. A subclass of UICollectionViewFlowLayout used in the collection view.
*/
private class DRCollectionViewLayout: UICollectionViewFlowLayout {
    var delegate: DRCollectionViewLayoutDelegate!
    var width: CGFloat!
    var midX: CGFloat!
...
// MARK: DRPickerViewDelegateIntercepter
/**
Private. Used to hook UICollectionViewDelegate and throw it DRPickerView,
and if it conforms to UIScrollViewDelegate, also throw it to DRPickerView's delegate.
*/
private class DRPickerViewDelegateIntercepter: NSObject, UICollectionViewDelegate {
    weak var pickerView: DRPickerView?
    weak var delegate: UIScrollViewDelegate?
    
    init(pickerView: DRPickerView, delegate: UIScrollViewDelegate?) {
...
*/
public class DRPickerView: UIView, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, DRCollectionViewLayoutDelegate {
    
    // MARK: - Properties
    // MARK: Readwrite Properties
    /// Readwrite. Data source of picker view.
    public weak var dataSource: DRPickerViewDataSource? = nil
    /// Readwrite. Delegate of picker view.
    public weak var delegate: DRPickerViewDelegate? = nil {
        didSet(delegate) {
    ...
    /// Readwrite. The style of the picker view. See DRPickerViewStyle.
    public var pickerViewStyle = DRPickerViewStyle.Wheel
    
    /// Readwrite. A float value which determines the perspective representation which used when using DRPickerViewStyle.Wheel style.
...
//                }()
    ...
    private var intercepter: DRPickerViewDelegateIntercepter!
    /// Private. A UICollectionViewFlowLayout used in picker view's collection view.
    private var collectionViewLayout: DRCollectionViewLayout {
        let layout = DRCollectionViewLayout()
    ...
    */
    private func initialize() {
        ...
        self.collectionView.registerClass(
            DRCollectionViewCell.self,
            forCellWithReuseIdentifier: NSStringFromClass(DRCollectionViewCell.self))
        self.addSubview(self.collectionView)
        
        self.intercepter = DRPickerViewDelegateIntercepter(pickerView: self, delegate: self.delegate)
    ...
    public override func intrinsicContentSize() -> CGSize {
        return CGSizeMake(UIScreen.mainScreen().bounds.width, 45)
    }
    ...
    */
    ...
    public func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCellWithReuseIdentifier(NSStringFromClass(DRCollectionViewCell.self), forIndexPath: indexPath) as! DRCollectionViewCell
        if let title = self.dataSource?.pickerView?(self, titleForItem: indexPath.item) {
            ...
            cell.label.bounds = CGRect(origin: CGPointZero, size: CGSize(width: UIScreen.mainScreen().bounds.width / 3, height: 45))
    ...
    public func collectionView(collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAtIndexPath indexPath: NSIndexPath) -> CGSize {
        var size = CGSizeMake(self.interitemSpacing, collectionView.bounds.size.height)
            ...
            size.width += UIScreen.mainScreen().bounds.width / 3
    ...
    // MARK: DRCollectionViewLayoutDelegate
    private func pickerViewStyleForCollectionViewLayout(layout: DRCollectionViewLayout) -> DRPickerViewStyle {
